...
from myipv6 import ipv6explod,ipv62hexstr
...
def get_oui():
    ouis = []
    lines = read_big_file('./hitlist/oui.txt')
    for line in lines:
        oui = line[:8].replace('-','')
        ouis.append(oui.lower())
    return ouis
...
def read_exploded_hitlist(file_name):
    a = read_big_file(file_name,remove_duplicates=True)
    if ':' in a[0]:
        a = [ipv62hexstr(x) for x in a]
