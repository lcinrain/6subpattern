...
import time
import redis
from TCPClient import GetTCPConnection
...
def DynamicScan(root, V, budget, source_ip, output_dir,r):
    """
    动态扫描空间树
    ...
    xi, budget, R, T = Scan_Feedback(xi, init_budget, budget, R, T, V, source_ip, output_dir, target_file,scan_round,r)
    print('round ',scan_round)
    scan_round+=1
    while budget > 0:
        xi_h = TakeOutFrontSegment(xi, int(0.1 * len(xi)))  # 每次迭代需要扫描的结点
        ReplaceDescendants(xi, xi_h)
        xi_h, budget, R, T = Scan_Feedback(xi_h, init_budget, budget, R, T, V, source_ip, output_dir, target_file,scan_round,r)
        print('round ',scan_round)
        scan_round+=1
...
def Scan_Feedback(xi, init_budget, budget, R, T, V, source_ip, output_dir, target_file,scan_round,r):
    """
    对队列xi中的所有结点进行一次扫描，
    ...
    #     for target in C:
    #         f.write(target + '\n')
    active_addrs = set(Scan(C, source_ip, output_dir, scan_round,r))   #扫描并得到活跃的地址集合
...
def Start(r,clientSocket):
    parse=argparse.ArgumentParser()
    parse.add_argument('--input', type=str, default='../hitlist_downsampling.compressed.10000.txt',help='input IPv6 address filename')
    parse.add_argument('--output',type=str,default='./test',help='output directory name')
    parse.add_argument('--budget',type=int,default=100000,help='the upperbound of scan times')
    parse.add_argument('--IPv6',type=str,default='',help='local IPv6 address')
    ...
    R, P, target_len, result_len, hit_rate = DynamicScan(root, V, args.budget, args.IPv6, args.output,r)
...
if __name__ == '__main__':
    ...
    clientSocket = None
    target, result, hit_rate = Start(r)
