...
def get_response_targets(targets,output=False,out_filename='',verbose=True):
    ...
    if output:
        ...
        write_list2file(responsive_targets,responsive_filename)
        
        unprobed_filename = file_name.replace('.txt','.unprobed.txt')
        write_list2file(unprobed_targets,unprobed_filename)
...
def hitrate_10M(targets_dir:str,range_start=0,range_end=0,filename_read_pattern='targets*.route.dealiased.noseed.txt',output_targets_unprobed=True,budget=10**7,rounds = 20,targets_input:list=[],hitrate_filename='log.hitrate.10Mbudget.txt',hitrate_mode='normal'):
    ...
    fw_log = open(targets_dir+hitrate_filename,'w')
    ...
    for i in range(0,math.ceil(len(targets)/budget_step)):
        ...
        if hitrate_mode == 'normal':
            hitrate = len(targets_responsive)/len(targets_seg) if (len(targets_seg)-len(targets_unprobed))>0 else 0
        else:
    ...
    if hitrate_mode == 'normal':
        loginfo = 'ave hitrate:%s, total resp %s, total targets %s, total unprobed %s\n'%(total_responsive/len(targets),total_responsive,len(targets),total_unprobed)
    else:
    ...
    for data in list_log_data:
        ...
        fw_log.write('\n')
    fw_log.close()
    return total_responsive/len(targets)
