...
from ActiveScan import Scan,get_currentime
...
def DynamicScan(root, args,r):
    ...
    while 1:
        ...
        print('scanning round: ',scan_round)
        xi, now_budget, R, T= Scan_Feedback(xi, budget, now_budget, R, T,  all_Reward, args,scan_round,r)
        scan_round+=1
...
def Scan_Feedback(xi, budget,now_budget, R, T, all_Reward, args,scan_round,r):
    """
    对队列xi中的所有结点进行一次扫描，
...
def Start(r):
    ...
    parse.add_argument('--input', type=str, default="../hitlist_biasedsampling.compressed.100000.txt",help='input IPv6 addresses')
    parse.add_argument('--output',type=str, default="./biased",help='output directory name')
    parse.add_argument('--budget',type=int, default=1000000, help='the upperbound of scan times')
    parse.add_argument('--IPv6',type=str, default="2401:c080:3800:2ec8:5400:4ff:fe78:2146", help='local IPv6 address')
    ...
    R, target_len, result_len, hit_rate = DynamicScan(root, args,r)
    print('Over!')
