...
def SeqToAddrs(node, number, args):
    ...
    addr_result_candidate = list(set(node.generated_address) - node.SS)
    if len(addr_result_candidate) > number:
        addr_result = random.sample(addr_result_candidate, number)
        return addr_result
    return addr_result_candidate
