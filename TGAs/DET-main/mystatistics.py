...
indices = [1,2,3]
all_targets = set()
fw = open('AddrminerS.hitrate.log.txt','w')
for sam in sampling:
    print(sam)
    fw.write(sam+'\n')
    for index in indices:
        ...
        b = budget_dict[index]
        fw.write(f'\nbudget {b}\n')
        file_name_target = f'./{sam}{index}/result/6density.target{b}'
        file_name_response = f'./{sam}{index}/result/6density.result{b}'
        ...
        i = 0
        header = ['round','target','dealiased','response_dealiased','hitrate','accu_res','accu_res_float']
        ...
        for target in r:
            ...
            num_accu_res+=response_num_roundi # accu_response
            info_list.append(str(num_accu_res))
            
            accu_float = (num_accu_res/K) if index<2 else (num_accu_res/M)
            info_list.append(str(accu_float))
