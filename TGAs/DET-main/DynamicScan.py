...
def DynamicScan(root, budget, source_ip, output_dir,r):
    """
    动态扫描空间树
    ...
    xi, budget, R, T = Scan_Feedback(xi, init_budget, budget, R, T, source_ip, output_dir, target_file,scan_round,r)
    scan_round+=1
    while budget > 0:
        xi_h = TakeOutFrontSegment(xi, int(0.1 * len(xi))+1)  # 每次迭代需要扫描的结点
        ReplaceDescendants(xi, xi_h)
        print('round ',scan_round)
        xi_h, budget, R, T = Scan_Feedback(xi_h, init_budget, budget, R, T, source_ip, output_dir, target_file,scan_round,r)
        scan_round+=1
...
def Scan_Feedback(xi, init_budget, budget, R, T, source_ip, output_dir, target_file,scan_round,r):
    """
    对队列xi中的所有结点进行一次扫描，
...
def Start(r):
    ...
    R, target_len, result_len, hit_rate = DynamicScan(root, args.budget, args.IPv6, args.output,r)
    print('Over!')
    # hit_rate = float(len(R))/(init_budget - budget)
